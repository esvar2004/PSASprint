...
function MapComponent() {
  ...
  const [allMarkers, setAllMarkers] = useState<CustomMarker[]>([]); // store all markers here
  const [visibleMarkers, setVisibleMarkers] = useState<CustomMarker[]>([]); // store currently visible markers
  ...
  useEffect(() => {
    ...
    setVisibleMarkers(newMarkers); // Show all markers initially
    mapReference.current?.fitBounds(markerBounds); // Fit all markers to map
  }, [isLoaded, mapReference.current]);
  ...
  const handleButtonClick = (type: string) => {
    ...
    const filteredMarkers = allMarkers.filter(({ type: markerType }) =>
      type === "all" ? true : markerType === type
    );
    ...
    // Adjust the map to fit the filtered markers
    if (filteredMarkers.length > 0) {
      const markerBounds = new google.maps.LatLngBounds();
      filteredMarkers.forEach(({ marker }) => {
        markerBounds.extend(marker.getPosition() as google.maps.LatLng);
      });
      mapReference.current?.fitBounds(markerBounds);
    } else {
      // If no markers to show, reset to a default view
      mapReference.current?.setCenter({ lat: 0, lng: 0 });
      mapReference.current?.setZoom(3);
    }
  }; 
  ...
  return (
    <div className="relative" style={{ width: "100%", height: "100vh" }}>
      ...
      {/* Buttons for filtering markers by type (color) */}
